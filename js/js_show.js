	...
	function send(problem) {
	    ...
	    if (request) {
	        ...
	        //var data = "app_key=daab3916-d808-4173-b875-a8a099ae6f15&user_id=123456&q=" + problem;
	        request.send();
	        request.onreadystatechange = function() { //监听连接状态
	            if (request.readyState == 4 && request.status == 200) {
	                var robotContent = JSON.parse(request.responseText); //
					var res = robotContent.result.intents;     //
					var res2 = res[0];
	                ...
	                robot_body.innerHTML = ' <div class="robot"><div class="avatar">' +
	                    ...
	                    '<span id="robot_contents">' + res2.result.text.toString() + '</span></div></div>';
